    ...
    if let Some(msg) = update.message {
        ...
        if let Some(ref text) = msg.text {
            ...
            match command {
                ...
                "/about" => handle_about(&bot, chat_id).await,
                ...
                "/textstyles" => handle_text_styles(&bot, chat_id).await,
                "/botinfo" => handle_bot_info(&bot, chat_id).await,
                "/webhookinfo" => handle_webhook_info(&bot, chat_id).await,
                "/membercount" => handle_member_count(&bot, chat_id).await,
                "/admins" => handle_admins(&bot, chat_id).await,
                "/invitelink" => handle_invite_link(&bot, chat_id).await,
                "/mycommands" => handle_my_commands(&bot, chat_id).await,
                "/myprofile" => handle_my_profile(&bot, chat_id, user_id).await,
                "/library" => handle_library(&bot, chat_id).await,
